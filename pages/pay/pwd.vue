	...
	<view>
		<view class="form-box">
			<u-form ref="uForm" label-width="130rpx" :model="form">
				<u-form-item label="原密码" prop="pwdOld" required>
					<u-input v-model="form.pwdOld" type="password" clearable placeholder="请输入原来的交易密码"></u-input>
				</u-form-item>
		...
		<view class="submit-btn">
			...
			<u-cell icon="question-circle" title="忘记交易密码？" value="重置" is-link url="/pages/pay/pwd-reset" />
		</view>
	</view>
</template>
	...
	export default {
		...
		methods: {
			...
			async _submit() {
				...
				const res = await this.$wxapi.modifyPayPassword(this.token, this.form.pwdOld, this.form.pwd)
				if (res.code != 0) {
					uni.showToast({
						title: res.msg,
						icon: 'none'
